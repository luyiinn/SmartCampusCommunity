<template>
  <el-dialog v-model="visibleProxy" title="发布日记" width="520px">
    <el-form label-width="80px" :model="formData" :rules="formRules" ref="formRef" class="publish-form">
      <!-- 标题输入框 -->
      <el-form-item label="标题" prop="title" class="form-item">
        <el-input v-model="formData.title" placeholder="请输入日记标题" maxlength="100" show-word-limit />
      </el-form-item>

      <!-- 可见性选择 -->
      <el-form-item label="可见性" prop="isPublic" class="form-item">
      ...
      <el-form-item label="内容" prop="content" class="form-item">
        <el-input v-model="formData.content" type="textarea" rows="6" placeholder="记录今日的思考与感悟..." />
      </el-form-item>

      <!-- 图片上传 -->
      <el-form-item label="图片" prop="images" class="form-item">
        <el-upload v-model:file-list="fileList" action="" :auto-upload="false" :on-change="handleImageChange"
          :before-upload="beforeImageUpload" list-type="picture-card" :limit="9" :on-exceed="handleExceed"
          ...
          <template #file="{ file }">
            <div class="image-upload-item">
              ...
              <div class="image-upload-mask">
                <el-icon class="image-upload-icon" @click.stop="handleRemove(file)">
                  <Delete />
                </el-icon>
              </div>
            </div>
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="isSubmitting" :disabled="isSubmitting || !isFormValid"
...
const emit = defineEmits<{
  ...
  submit: [{ title: string; content: string; isPublic: boolean; images: string[] }];
}>();
...
const handleSubmit = async () => {
  try {
    ...
    // 发送请求到/diary/add接口
    const response = await axios.post("/diary/add", requestData, {
      headers: {
    ...
    if (response.data && response.data.code === 1) {
      ...
      ElMessage.success(response.data.data || "日记发布成功");
